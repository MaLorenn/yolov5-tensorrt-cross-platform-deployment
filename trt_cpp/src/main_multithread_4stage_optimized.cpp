...
int main(int argc, char** argv)
{
    ...
    // 摄像头帧队列容量为1，满时丢弃旧帧。
    constexpr std::size_t captureQueueCapacity = 1;

    // 预处理结果已经付出了较大计算成本，因此这里使用阻塞队列，
    // 不轻易丢弃已经完成预处理的数据。
    constexpr std::size_t preprocessQueueCapacity = 1;

    constexpr std::size_t resultQueueCapacity = 1;
    ...
    try
    {
        ...
        BoundedQueue<CapturedFrame>
            frameQueue(captureQueueCapacity);

        BoundedQueue<PreprocessedFrame>
            preprocessQueue(preprocessQueueCapacity);

        BoundedQueue<InferenceResult>
            resultQueue(resultQueueCapacity);
        ...
        std::thread captureThread(
            [&]()
            {
                ...
                while (processedFrames.load() < maxFrames)
                {
                    ++frameId;
        ...
        std::thread preprocessThread(
            [&]()
            {
                ...
                while (frameQueue.pop(capturedFrame))
                {
                    ...
                    preprocessedFrame.preprocessResult =
                        std::move(preprocessResult);

                    preprocessedFrame.captureTime =
                        capturedFrame.captureTime;

                    preprocessedFrame.preprocessMilliseconds =
                        preprocessMilliseconds;

                    preprocessedFrame.frameId =
                        capturedFrame.frameId;

                    // 预处理结果已经消耗了计算时间，
                    // 使用普通阻塞push，避免随意丢弃。
                    if (!preprocessQueue.pushDropOldest(
                            std::move(preprocessedFrame)))
                    {
                        break;
                    }
                }

                preprocessQueue.stop();
            });
        ...
        std::thread resultThread(
            [&]()
            {
                ...
                while (resultQueue.pop(result))
                {
                    ...
                    const std::size_t currentProcessed =
                        ++processedFrames;

                    if (currentProcessed % 30 == 0)
                    {
                        std::cout
                            << "已处理："
                            << currentProcessed
        ...
        const std::size_t finalProcessedFrames =
            processedFrames.load();
